...
class AudioFeatureExtractor:
    ...
    def __init__(
        ...
        self.cache_dir = (
            Path(cache_dir) if cache_dir else Path.home() / ".llama_voice" / "models"
        )

        # Check dependencies
        if self.use_mlx and not HAS_MLX:
            self.logger.warning(
                "MLX not available. Install with 'pip install mlx mlx-audio'"
            )
            self.use_mlx = False
        ...
        self.logger.info(
            f"Audio Feature Extractor initialized with {self.feature_type.value}"
        )
    ...
    def _init_custom_model(self) -> None:
        ...
        if not self.model_path:
            self.logger.warning(
                "Custom feature type selected but no model path provided"
            )
            return
        ...
        try:
            ...
            self.logger.info(
                f"Loading custom feature extraction model from {model_path}"
            )
            # self.custom_model = ...
    ...
    def extract(
        ...
        if config_override:
            ...
            config_dict = {
                k: getattr(current_config, k)
                for k in current_config.__dataclass_fields__
            }
        ...
        try:
            ...
            if extract_type == FeatureType.MEL_SPECTROGRAM:
                features = self._extract_mel_spectrogram(
                    audio_array, sample_rate, current_config
                )
                ...
                features = self._extract_filterbank(
                    audio_array, sample_rate, current_config
                )
    ...
    def _extract_mel_spectrogram(
        ...
        if self.use_mlx:
            ...
            if config.pre_emphasis:
                audio = mlx_audio.transforms.preemphasis(
                    audio, coefficient=config.pre_emphasis
                )

            # Add dither if configured
            if config.dither > 0:
                noise = mx.random.uniform(
                    low=-config.dither, high=config.dither, shape=audio.shape
                )
                audio = audio + noise
            ...
            if HAS_LIBROSA:
                ...
                # Normalize if configured
                if config.normalize:
                    mel_spec = (mel_spec - np.mean(mel_spec)) / (
                        np.std(mel_spec) + 1e-8
                    )
    ...
    def _extract_mfcc(
        ...
        if self.use_mlx:
            ...
            if HAS_LIBROSA:
                ...
                raise ImportError(
                    "Neither MLX nor librosa is available for MFCC extraction"
                )
    ...
    def _extract_filterbank(
        ...
        if self.use_mlx:
            ...
            if HAS_LIBROSA:
                ...
                raise ImportError(
                    "Neither MLX nor librosa is available for filterbank extraction"
                )
