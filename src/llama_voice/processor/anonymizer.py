...
class VoiceprintAnonymizer:
    ...
    def __init__(
        ...
        if self.anonymizer_type == AnonymizerType.NEURAL_ANONYMIZER and not HAS_MLX:
            self.logger.warning(
                "MLX not available. Neural anonymizer will not be available."
            )
        ...
        self.logger.info(
            f"Voiceprint Anonymizer initialized with {self.anonymizer_type.value}"
        )
    ...
    def _init_anonymization_params(self) -> None:
        ...
        if self.anonymizer_type == AnonymizerType.PITCH_SHIFT:
            ...
            if not self.config.target_voice_id:
                ...
                self.anonymization_params["target_voice_id"] = (
                    f"voice_{np.random.randint(1, 10)}"
                )
            else:
                self.anonymization_params["target_voice_id"] = (
                    self.config.target_voice_id
                )
    ...
    def anonymize(
        ...
        should_preserve_prosody = (
            preserve_prosody
            if preserve_prosody is not None
            else self.config.preserve_prosody
        )
        strength = (
        ...
        try:
            ...
            if self.anonymizer_type == AnonymizerType.PITCH_SHIFT:
                ...
                self.anonymizer_type == AnonymizerType.NEURAL_ANONYMIZER
                and self.neural_model
            ):
    ...
    def _anonymize_voice_conversion(
        ...
        if self.neural_model:
            ...
            self.logger.warning(
                "Voice conversion not available, falling back to pitch shift"
            )
            return self._anonymize_pitch_shift(audio)
    ...
    def _anonymize_neural(
        ...
        if not self.neural_model or not HAS_MLX:
            self.logger.warning(
                "Neural anonymizer not available, falling back to pitch shift"
            )
            return self._anonymize_pitch_shift(audio)
