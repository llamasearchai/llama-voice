...
class SecureEnclaveManager:
    ...
    def __init__(
        ...
        if not self.has_secure_enclave:
            self.logger.warning(
                "Secure Enclave not available. Falling back to encrypted storage."
            )
        ...
        self.logger.info(
            f"Secure Enclave Manager initialized with storage at {self.storage_path}"
        )
    ...
    def _decrypt_with_secure_enclave(self, encrypted_data: bytes) -> bytes:
        ...
        try:
            ...
            decrypted = bytes(
                a ^ b
                for a, b in zip(encrypted_data, key_extended[: len(encrypted_data)])
            )
