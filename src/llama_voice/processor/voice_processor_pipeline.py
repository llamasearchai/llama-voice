...
class VoiceProcessor:
    ...
    def _init_components(self) -> None:
        ...
        self.secure_enclave = (
            SecureEnclaveManager() if self.enable_secure_enclave else None
        )
        ...
        try:
            ...
            if isinstance(audio, (str, Path)):
                ...
                if sample_rate is None:
                    raise ValueError(
                        "Sample rate must be provided for numpy array input"
                    )
            ...
            if callback:
                callback(
                    "asr_complete", {"text": transcription, "confidence": confidence}
                )
            ...
            if detect_watermark:
                ...
                if callback:
                    callback(
                        "watermark_detection_complete", {"detected": watermark_detected}
                    )
            ...
            if self.enable_secure_enclave and self.secure_enclave:
                if result.watermark_detected or (
                    ...
                    t0 = time.time()
                    self.secure_enclave.store_sensitive_audio(
                        audio_segment, result.transcription
                    )
                    component_timings["secure_storage"] = time.time() - t0

        except Exception as e:
            self.logger.error(
                f"Error in voice processing pipeline: {str(e)}", exc_info=True
            )
            raise
